...
export default function InviteClient({ code }) {
  ...
  const safeCode = (code ?? "[NO_CODE]").toString();
  // DEBUG (temporär):
  // eslint-disable-next-line no-console
  console.log("InviteClient received code:", code);

  const looksValid = useMemo(() => basicLooksValid(safeCode), [safeCode]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/invites/${encodeURIComponent(safeCode)}`, {
          cache: "no-store",
        });
  ...
  async function joinGroup() {
    ...
    try {
      const res = await fetch(`/api/invites/${encodeURIComponent(safeCode)}/redeem`, {
        method: "POST",
        credentials: "same-origin",
        cache: "no-store",
      });
    ...
    <main className="relative min-h-screen text-white pt-20">
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-6 pt-10 pb-16">
        <div className="max-w-2xl">
          <p className="text-xs uppercase tracking-widest text-white/60">Invite</p>
          ...
          <p className="mt-4 text-white/70 md:text-lg">
            Öffne die App, um der Gruppe beizutreten – oder installiere sie, falls du sie noch nicht hast.
          </p>
          ...
          <div className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-2 font-mono text-2xl">{safeCode}</div>
              </div>
            ...
            {!loading && valid && groupInfo?.groupName && (
              <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                <div className="text-xs uppercase tracking-widest text-white/50">Gruppe</div>
                <div className="mt-1 text-sm font-semibold text-white">{groupInfo.groupName}</div>
                {typeof groupInfo.memberCount === "number" && (
                  <div className="mt-1 text-xs text-white/60">{groupInfo.memberCount} Mitglieder</div>
                )}
              </div>
            )}
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              {!loggedIn ? (
                <a
                  href={`/login?next=${encodeURIComponent(`/invite/${safeCode}`)}`}
                  className={[
